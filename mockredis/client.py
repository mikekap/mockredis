...
class MockRedis(object):
    ...
    def expire(self, key, delta):
        ...
        return self.pexpireat(key, self.clock.now() + delta)

    def pexpire(self, key, delta):
        """Emulate pexpire"""
        delta = delta if isinstance(delta, timedelta) else timedelta(milliseconds=delta)
        return self.pexpireat(key, self.clock.now() + delta)

    def expireat(self, key, when):
        if not isinstance(when, datetime):
            when = datetime.fromtimestamp(when / 1000.0)
        return self.pexpireat(key, when)
    ...
    def pttl(self, key):
        ...
        get_result = get_total_milliseconds
    ...
    def hset(self, hashkey, attribute, value):
        ...
        was_there = attribute in redis_hash
        redis_hash[attribute] = self._encode(value)
        return 0 if was_there else 1
    ...
    def lrem(self, key, value, count=0):
        ...
        if self._encode(key) in self.redis:
            reverse = count < 0
            count = sys.maxsize if count == 0 else (count if count > 0 else -count)
